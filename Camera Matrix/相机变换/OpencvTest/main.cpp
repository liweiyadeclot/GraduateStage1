...
void main()
{
	...
	while (getline(fin, filename))
	{
		...
		else
		{
			...
			cv::namedWindow("Camera Calibration", cv::WINDOW_NORMAL);  // 创建一个可调整大小的窗口
			cv::resizeWindow("Camera Calibration", 1280, 960);  // 调整窗口大小为640x480
	...
	for (int i = 0; i < image_count; i++)
	{
		...
		if (i != 0)
		{
			...
			fout << "第" << i + 1 << "个相机对第一个相机的旋转矩阵" << endl;
			fout << RMatFromCurrCameraToFirstCamera << endl;

			fout << "第" << i + 1 << "个相机对第一个相机的平移向量" << endl;
